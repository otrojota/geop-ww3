...
class ProveedorCapasWW3 extends ProveedorCapas {
    ...
    async generaMatrizUV(args) {
        try {
            ...
            if (capa.opciones.vector) {
                ...
                let ret = await this.generaMatrizRectangular(argsCapaDireccion);
                ret.data = [];
                ret.rows.forEach(row => {
                    row.forEach(v => ret.data.push(v));
                })
    ...
    async generaWindGLPNG(args) {
        try {
            ...
            if (capa.opciones.vector) {
                ...
                let [rU, rV] = await(
                    Promise.all([
                        gdal.getRectangularMatrix(b.lng0, b.lat0, b.lng1, b.lat1, path, bandaU, width, height, config.publishPath, varMetadataU),
                        gdal.getRectangularMatrix(b.lng0, b.lat0, b.lng1, b.lat1, path, bandaV, width, height, config.publishPath, varMetadataV)
                    ])
                );
                let dataU = rU.data;
                let dataV = rV.data;
                ret.lng0 = rU.box.newLng0;
                ret.lng1 = rU.box.newLng1;
                ret.lat0 = rU.box.newLat0;
                ret.lat1 = rU.box.newLat1;
                ...
                let r = await gdal.getRectangularMatrix(b.lng0, b.lat0, b.lng1, b.lat1, path, metadataDireccion.banda, width, height, config.publishPath, metadataDireccion);
                let direccion = r.data;
                ret.lng0 = r.box.newLng0;
                ret.lng1 = r.box.newLng1;
                ret.lat0 = r.box.newLat0;
                ret.lat1 = r.box.newLat1;
    ...
    async generaMatrizRectangular(args, interpolacion) {
        try { 
            ...
            let banda = varMetadata.banda;
            let path = this.getPath(time) + "/" + time.format("DD_HH00") + ".grb2";
            let r = await gdal.getRectangularMatrix(b.lng0, b.lat0, b.lng1, b.lat1, path, banda, width, height, config.publishPath, varMetadata, interpolacion);
            let data = r.data;
            ...
            data.lng0 = r.box.newLng0; data.lng1 = r.box.newLng1;
            data.lat0 = r.box.newLat0; data.lat1 = r.box.newLat1;
